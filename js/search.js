...
// Function to filter the courses based on category and search keyword
function filterCourses(category, searchKeyword) {
  var filteredCourses = []; // Initialize the filteredCourses array
  ...
  if (searchKeyword) {
    filteredCourses = filteredCourses.filter(function (course) {
      return (
        course.courseTitle.toLowerCase().includes(searchKeyword.toLowerCase()) ||
        course.courseCategory.toLowerCase().includes(searchKeyword.toLowerCase())
      );
    });
  }

  // Display the filtered courses on the main index page
  document.getElementById('course-grid').innerHTML =
    '<ul class="course-list">' +
    filteredCourses.map(function (course) {
      return '<li><a href="course.html?id=' + course.id + '">' + course.courseTitle + '</a></li>';
    }).join('') +
    '</ul>';
}
